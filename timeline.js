...
export function postUnsubscribedChannel( {type = "",channelId ="", city="",school="",college=""} = {}){
...
export function postChannelForNewUser ({userId ="", educations=[],city=""} = {}){
...
export function postUnreadMessage({userId="",messageId="",channelId=""} = {}){
  ...
  if(userId){
    try{
    var r = await redisConn.zadd(config.redisPrefix.sortedSet.userUnreadMessageByUserId+userId,time(),messageId);
    }catch(e){
    f(e);
    return;
    }
...
export function postMessage ({userId="",messageId="",channelId=""} = {}){
